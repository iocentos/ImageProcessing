...
int main( int argc , char** argv ){ 
	...
	std::cout<<"**************************************************************"<<std::endl;
	std::cout<<"*              CilkPlus execution with "<<__cilkrts_get_nworkers()<<" threads             *"<<std::endl;
	std::cout<<"**************************************************************"<<std::endl;
	...
	unsigned int parallelImages = __cilkrts_get_nworkers();
	...
	try {
		...
		for( std::vector<std::string>::iterator it = imageName.begin() ; it < imageName.end() ; it += parallelImages ){
			...
			cilk_for( unsigned int i = 0 ; i < parallelIterations ; i++ ) {
